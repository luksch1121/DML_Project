...
def main():
    ...
    for i in range(num_episodes):
        for batch in train_dataset:
            ...
            # response_texts = tokenizer.decode(responses.squeeze(), skip_special_tokens=True)
            response_texts = tokenizer.decode(responses[:, queries.shape[-1]:][0], skip_special_tokens=True)
            reward = reward_model(response_texts)[0]["score"]
